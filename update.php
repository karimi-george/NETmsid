...
// To update the status of the voucher being used.
...
while ($datadb_voucher = mysqli_fetch_assoc($checkdb_voucher)) {
    ...
    if ($datadb_voucher['status_voucher'] == 'Sudah Digunakan') {
        echo "<font color='green'><b>Voucher : $voucher | Status : Already used  </font><br><br>";
    } else {
        // cek data router 
        ...
        if ($API->connect($ip_server, $username, $password)) {
            ...
            foreach ($hotspotactive as $active_voucher) {
                ...
                if ($voucherdb == $voucher_actived) {
                    $update = mysqli_query($db, "UPDATE `orders` SET `status_voucher`='Already used' WHERE voucher='$voucher_actived'");
                    $update = mysqli_query($db, "INSERT INTO report (service, voucher, price, date ) VALUES ('$layanan','$voucherdb','$harga','$date') ");
                    if ($update == TRUE) {
                        echo "<font color='green'><b>Voucher : $voucher | Status : Successfully Updated  </font><br><br>";
                    } else {
                        echo "System Error";
                    }
                }
            }
            echo "<font color='red'><b>Voucher : $voucher | Status : Not yet active on hotspot proxy </font><br><br>";
        }
    }
}
